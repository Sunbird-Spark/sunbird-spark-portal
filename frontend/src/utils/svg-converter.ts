...
const extractFileName = (template: string): string => {
    ...
    try {
        ...
        return fileName && fileName.indexOf('.svg') === -1 ? 'certificate' : (fileName?.split('.svg')[0] || 'certificate');
    } catch (_e) {
        return 'certificate';
    }
};
...
export const convertSvgToOutput = async (
    ...
    if (typeof input === 'string') {
        ...
        if (!config.fileName) {
            config.fileName = extractFileName(input);
        }

        element = appendGhostDiv(
